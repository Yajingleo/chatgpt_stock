...
def main():
    ...
    if not server_file.exists():
        print("❌ Error: web/server.py not found!")
        return
...
def launch_default():
    ...
    try:
        subprocess.run([sys.executable, "-m", "web.server"], check=True)
...
def launch_custom():
    ...
    cmd = [sys.executable, "-m", "web.server", "--host", host, "--port", port]
    if no_browser:
...
def launch_dev_mode():
    ...
    try:
        subprocess.run([sys.executable, "-m", "web.server", "--no-browser"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"❌ Error launching server: {e}")
    except KeyboardInterrupt:
        print("\n🛑 Server stopped.")
...
def show_help():
    ...
    print()
    print("🔧 Manual Launch:")
    print("python -m web.server [options]")
    print()
    print("Options:")
